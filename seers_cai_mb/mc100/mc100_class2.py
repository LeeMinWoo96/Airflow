...
#             print ('')
    ...
    def get_list(self, status):
        ...
        self.get_list_ecg(self.list_measurement) ## 여기서 해주네 추가를 
    ...
    def download_ecg(self, list_measurement):
        ...
        for idx, measurement in enumerate(list_measurement):
            ...
            if measurement['ecgDataList'] is None :
                continue
            path = os.path.join(MC100.DOWNLOAD_PATH, measurement_code) # 다운받을 경로 현재는 내 로컬
            measurement['storePath'] = path
            for idx, ecg in enumerate(measurement['ecgDataList']):
                ecg_id = ecg['ecgId']
                URL_API_DOWNLOAD_ECG = MC100.URL_BASE + '/Measurement/DownloadECGFile/{code}:{id}'.format(code=measurement_code, id=str(ecg_id)) # 파일 url 
